...
from keras.layers import Conv1D, Dense, Flatten, LSTM, Bidirectional, Input, Masking, Dropout, SimpleRNN
...
def cnn_model():
    ...
    model.add(Conv1D(512, 5, activation="linear"))
    model.add(Conv1D(512, 5, activation="linear"))
...
def rnn_model():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(SimpleRNN(512, activation="linear", return_sequences=True))
    model.add(SimpleRNN(512, activation="linear"))
    model.add(Dropout(0.5))
    model.add(Dense(512, activation="linear"))
    # model.add(Dropout(0.5))
    model.add(Dense(3, activation="softmax"))
...
def main():
    ...
    for clf in classifiers:
        ...
        if isinstance(clf, Sequential):
            ...
            for _ in tqdm(range(7), desc=f"Fitting {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
