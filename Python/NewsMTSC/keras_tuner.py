...
def cnn_model():
    ...
    model.add(Conv1D(filters=512, kernel_size=10, strides=5))
    model.add(Conv1D(filters=512, kernel_size=10, strides=5))
...
def main():
    ...
    classifiers = [
        # VADER(),
        # SVC(C=0.96, kernel="rbf", gamma="scale"),
        # cnn_model(),
        # gru_model(),
        # bi_gru_model(),
        # lstm_model(),
    ...
    for clf in classifiers:
        ...
        if isinstance(clf, VADER):
            ...
            for _ in tqdm(range(1), desc=f"Processing {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
                ...
                iteration_losses.append("---")
        elif isinstance(clf, Sequential):
            for _ in tqdm(range(10), desc=f"Processing {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
                loss = clf.fit(train_data_tf, train_labels_one_hot, verbose=0, batch_size=71).history['loss'][0]
