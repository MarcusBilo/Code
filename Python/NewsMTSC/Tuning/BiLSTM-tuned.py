...
from keras.layers import Dense, Flatten, Masking, Dropout, LSTM, Bidirectional
...
def bilstm_11_best():
    ...
    model.add(Bidirectional(LSTM(units=160)))
    model.add(Dropout(0.1))
    model.add(Dense(units=128, activation="linear"))
    model.add(Dropout(0.1))
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.004)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_11_best"
    return model


def bilstm_11_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    ...
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0036)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_11_sec_best"
    return model
...
def bilstm_12_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(LSTM(units=224)))
    model.add(Dropout(0.1))
    model.add(Dense(units=32, activation="linear"))
    model.add(Dropout(0.5))
    model.add(Dense(units=992, activation="linear"))
    model.add(Dropout(0.4))
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0007)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_12_best"
    return model


def bilstm_12_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(LSTM(units=384)))
    model.add(Dropout(0.3))
    ...
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_12_sec_best"
    return model
...
def bilstm_21_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(LSTM(units=384, return_sequences=True)))
    model.add(Dropout(0.3))
    model.add(Bidirectional(LSTM(units=416)))
    model.add(Dropout(0.1))
    model.add(Dense(units=288, activation="linear"))
    model.add(Dropout(0.5))
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0003)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_21_best"
    return model


def bilstm_21_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    ...
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_21_sec_best"
    return model
...
def bilstm_22_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(LSTM(units=416, return_sequences=True)))
    model.add(Dropout(0.1))
    model.add(Bidirectional(LSTM(units=896)))
    model.add(Dropout(0.1))
    ...
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0011)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiLSTM_22_best"
    return model
...
def main():
    ...
    for clf in classifiers:
        iteration_losses = []
        epoch = 0
        if clf._name == "BiLSTM_12_best" or clf._name == "BiLSTM_12_sec_best" or clf._name == "BiLSTM_21_best" or clf._name == "BiLSTM_21_sec_best":
        ...
        elif clf._name == "BiLSTM_11_best" or clf._name == "BiLSTM_11_sec_best" or clf._name == "BiLSTM_22_best" or clf._name == "BiLSTM_22_sec_best":
            highest_test_accuracy = 0.0
            for _ in tqdm(range(25), desc=f"Processing {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
                epoch += 1
