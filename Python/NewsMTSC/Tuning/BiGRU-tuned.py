...
from keras.layers import Dense, Flatten, Masking, Dropout, GRU, Bidirectional
...
def bilgru_11_best():
    ...
    model.add(Bidirectional(GRU(units=96)))
    ...
    model._name = "BiGRU_11_best"
    return model


def bilgru_11_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(GRU(units=448)))
    model.add(Dropout(0.2))
...
def bilgru_12_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(GRU(units=288)))
    model.add(Dropout(0.5))
    model.add(Dense(units=672, activation="relu"))
    model.add(Dropout(0.5))
    ...
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiGRU_12_best"
    return model


def bilgru_12_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(GRU(units=96)))
    model.add(Dropout(0.3))
...
def bilgru_21_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(GRU(units=384, return_sequences=True)))
    model.add(Dropout(0.5))
    model.add(Bidirectional(GRU(units=576)))
    ...
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.001)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiGRU_21_best"
    return model


def bilgru_21_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
    model.add(Bidirectional(GRU(units=96, return_sequences=True)))
    model.add(Dropout(0.2))
    ...
    model.add(Flatten())
    model.add(Dense(3, activation="softmax"))
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0039)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiGRU_21_sec_best"
    return model
...
def bilgru_22_best():
    ...
    model.add(Dropout(0.2))
    model.add(Bidirectional(GRU(units=672)))
    model.add(Dropout(0.2))
    ...
    adam = tf.keras.optimizers.legacy.Adam(learning_rate=0.0002)
    model.compile(optimizer=adam, loss=categorical_crossentropy, metrics=CategoricalAccuracy())
    model._name = "BiGRU_22_best"
    return model


def bilgru_22_sec_best():
    model = Sequential()
    model.add(Masking(mask_value=0))
...
def main():
    ...
    for clf in classifiers:
        iteration_losses = []
        epoch = 0
        if clf._name == "BiGRU_11_best" or clf._name == "BiGRU_12_sec_best" or clf._name == "BiGRU_21_best" or clf._name == "BiGRU_22_best":
        ...
        elif clf._name == "BiGRU_12_best" or clf._name == "BiGRU_22_sec_best":
            highest_test_accuracy = 0.0
            for _ in tqdm(range(25), desc=f"Processing {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
                epoch += 1
                ...
                if test_accuracy > highest_test_accuracy and test_accuracy > 0.5:
                    ...
                    filename = f"{clf._name}_{rounded_accuracy}_e{epoch}_weights.h5"
                    clf.save_weights(filename)
        elif clf._name == "BiGRU_11_sec_best" or clf._name == "BiGRU_21_sec_best":
            highest_test_accuracy = 0.0
            for _ in tqdm(range(25), desc=f"Processing {getattr(clf, 'name', clf.__class__.__name__)}", unit="Epoch"):
                epoch += 1
