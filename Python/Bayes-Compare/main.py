...
# ------------------------------------------------------------------
# Use "signrank" to compare classifiers tested on multiple data sets
# Use "correlated_ttest" to compare two classifiers on a single data
# ------------------------------------------------------------------

# 🧠 Summary of setup:
# - Usage of stratified cross-validation, possibly with random shuffling. Repeated multiple times.
# - As a result, the training/testing splits vary across repetitions, so:
#   - One does not have one fixed, aligned set of test samples.
#   - One can not guarantee pairwise correspondence of predictions across runs.

# Because the samples evaluated by the model in each fold / repetition are not aligned across models, the "correlated_ttest" looses the pairing it relies on.
# In this case, due to repeated randomized cross-validation, the sets of instances used for evaluation differ each time
# unless tightly controlled random seeds with ensured identical splits
# This matches the assumptions of "signrank":
# - You have N performance scores from method A
# - You have N performance scores from method B
# - The scores are not instance-wise aligned, but you can compare them as independent samples

# ✅ Therefore, in this scenario:
# Use "signrank", because aggregated performance across multiple cross-validation runs without one-to-one aligned test sets are compared.
...
def signrank(x, rope):
    samples = signrank_MC(x, rope, prior_strength=0.5, prior_place="ROPE", nsamples=50000)
    winners = numpy.argmax(samples, axis=1)
    pl, pe, pr = numpy.bincount(winners, minlength=3) / len(winners)
    return pl, pe, pr


def correlated_ttest(x, rope):
    ...
    n = len(diff)
    x = numpy.mean(diff)
    var = numpy.var(diff, ddof=1) * (1 / n + 1 / (n - 1))
    if var == 0:
        return int(x < rope), int(-rope <= x <= rope), int(rope < x)
    pr = 1 - stats.t.cdf(rope, n - 1, x, numpy.sqrt(var))
    pl = stats.t.cdf(-rope, n - 1, x, numpy.sqrt(var))
    pe = 1 - pl - pr
    return pl, pe, pr
...
for row_system in systems:
    ...
    for col_system in systems:
        if row_system == col_system:
            ...
            row[col_system] = f"{results[(row_system, col_system)][0]:.3f}"
        elif (col_system, row_system) in results:
            row[col_system] = f"{results[(col_system, row_system)][2]:.3f}"
        else:
            row[col_system] = "N/A"
    grid[row_system] = row
for row_system in systems:
